...
float3 calculateSpecular(in float4 albedo)
{
    ...
    if (float(1) == 4)
    {
        ...
        if (float(1) == 2) // Toon
        {
            finalSpecular += calculateToonSpecular(albedo, poiMesh.uv[0], float4(0.04964832,0.1096813,0,1), metallic, float(1), specularMap, specularLightColor, poiMesh.normals[float(1)], poiLight.halfDir, poiLight.attenuation);
        ...
        if (float(1) == 3) // anisotropic
        {
            ...
            jitter += float(0);
            float4 packedTangentMap = POI2D_SAMPLER_PAN(_AnisoTangentMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0));
            finalSpecular += AnisotropicSpecular(float(0), float(0), float(1), float(0.881), float(0.169), float(0.037), float4(1,1,1,1), metallic, specularMap, specularLightColor, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
            finalSpecular *= float4(1,1,1,1);
            finalSpecular *= attenuation;
        }
    #endif
