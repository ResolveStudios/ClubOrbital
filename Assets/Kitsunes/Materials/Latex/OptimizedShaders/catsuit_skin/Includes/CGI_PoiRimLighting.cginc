...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimBrighten = float(0.13);
    float rimWidth = lerp( - .05, 1, float(0.409));
    #ifdef POI_AUDIOLINK
    ...
    rimLightEmission = rim * lerp(albedo, rimColor, float(0.431)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, float(0.431)) + lerp(albedo.rgb, rimColor, float(0.431)) * rimBrighten, rim);
}
#endif
