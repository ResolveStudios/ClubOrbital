...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimStrength = float(1.13);
    float rimBrighten = float(0);
    float rimWidth = lerp( - .05, 1, float(0.683));
    #ifdef POI_AUDIOLINK
    ...
    #endif
    #if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
        rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float4(0.0003035267,0.3721299,0.9386859,1);
    #else
        rimColor = float4(0.0003035267,0.3721299,0.9386859,1);
    #endif
    ...
    rim *= float4(0.0003035267,0.3721299,0.9386859,1).a * rimColor.a * rimMask;
    rimLightEmission = rim * lerp(albedo, rimColor, float(0.07)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, float(0.07)) + lerp(albedo.rgb, rimColor, float(0.07)) * rimBrighten, rim);
}
#endif
