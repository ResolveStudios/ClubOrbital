...
float3 calculateEmissionNew(in float3 baseColor, inout float4 finalColor)
{
    ...
    if (float(1))
    {
        emissionStrength0 *= calculateBlinkingEmission(float(0.5), float(2), float(4), float(0));
    }
    emissionColor0 = hueShift(emissionColor0, frac(float(0.673) + float(0) * _Time.x) * float(1));
    #if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
