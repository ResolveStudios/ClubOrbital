...
void initTextureData(inout float4 albedo, inout float4 mainTexture, inout float3 backFaceEmission, inout float3 dissolveEmission, in half3 detailMask)
{
    ...
    mainTexture.a *= alphaMask;
        ...
        albedo = float4(mainTexture.rgb * max(float4(0.03688945,0.01938235,0.07421356,1).rgb, float3(0.000000001, 0.000000001, 0.000000001)) * lerp(1, vertexColor, float(0)), mainTexture.a * max(float4(0.03688945,0.01938235,0.07421356,1).a, 0.0000001));
        ...
        applyBackFaceTexture(backFaceDetailIntensity, mixedHueShift, albedo, backFaceEmission);
            ...
            #endif
            albedo.rgb *= LerpWhiteTo(detailTexture * float(1) * unity_ColorSpaceDouble.rgb, detailMask.r * float(1.02) * backFaceDetailIntensity);
        #endif
