...
UnityLight CreateLight(float3 normal, fixed detailShadowMap)
{
    ...
    light.color = saturate(_LightColor0.rgb * lerp(1, poiLight.attenuation, float(0.68)) * detailShadowMap);
...
float FadeShadows(float attenuation)
{
            ...
            attenuation = lerp(1, poiLight.attenuation, float(0.68));
        #endif
...
void ApplySubtractiveLighting(inout UnityIndirect indirectLight)
{
        ...
        poiLight.attenuation = FadeShadows(lerp(1, poiLight.attenuation, float(0.68)));
...
void calculateBasePassLightMaps()
{
        ...
        float bw_directLighting = (((poiLight.nDotL * 0.5 + 0.5) * bw_lightColor * lerp(1, poiLight.attenuation, float(0.68))) + dot(ShadeSH9Normal(poiMesh.normals[1]), grayscale_vector));
        ...
        if (float(1) == 2) // Wrapped
        {
            ...
            float directatten = lerp(1, poiLight.attenuation, float(0.68));
