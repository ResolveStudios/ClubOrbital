...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimWidth = lerp( - .05, 1, float(0.8));
        ...
        if (poiMods.audioLinkTextureExists)
        {
            ...
            rimStrength += lerp(float4(0,2,0,0).x, float4(0,2,0,0).y, poiMods.audioLink[float(0)]);
            rimBrighten += lerp(float4(0,3,0,0).x, float4(0,3,0,0).y, poiMods.audioLink[float(2)]);
        }
    #endif
    rimWidth -= rimNoise;
    #if defined(PROP_RIMMASK) || !defined(OPTIMIZER_ENABLED)
    ...
    #endif
    #if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
        rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float4(0.5720443,0.02015762,1,1);
    #else
        rimColor = float4(0.5720443,0.02015762,1,1);
    #endif
    ...
    rim *= float4(0.5720443,0.02015762,1,1).a * rimColor.a * rimMask;
