...
float3 calculateEmissionNew(in float3 baseColor, inout float4 finalColor)
{
    ...
    float3 emissionColor0 = 0;
        ...
        if (poiMods.audioLinkTextureExists)
        {
            ...
            emissionStrength0 += lerp(float4(0,3,0,0).x, float4(0,3,0,0).y, poiMods.audioLink[float(1)]);
    ...
    float glowInTheDarkMultiplier0 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0));
        ...
        if (!float(0))
        {
            emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)).rgb * lerp(1, baseColor, float(0)).rgb * float4(0.4316379,0.03516899,0.617,0).rgb;
        }
        else
        {
            emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, baseColor, float(0)).rgb * float4(0.4316379,0.03516899,0.617,0).rgb;
        }
    #else
        emissionColor0 = lerp(1, baseColor, float(0)).rgb * float4(0.4316379,0.03516899,0.617,0).rgb;
    #endif
    ...
    #if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
        float emissionMask0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask, _MainTex, TRANSFORM_TEX(poiMesh.uv[float(0)], _EmissionMask) + _Time.x * float4(-5,0,0,0)).r;
    #else
    ...
    float3 emissionColor1 = 0;
        ...
        emissionStrength1 = float(0);
            ...
            if (poiMods.audioLinkTextureExists)
            {
                ...
                emissionStrength1 += lerp(float4(0,2,0,0).x, float4(0,2,0,0).y, getBandAtTime(float(1), saturate(1 - poiLight.nDotV), float(1)));
        ...
        float glowInTheDarkMultiplier1 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0));
            ...
            if (!float(0))
            {
                emissionColor1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * lerp(1, baseColor, float(0)).rgb * float4(0.07450981,0.01568627,0.06623452,1).rgb;
            }
            else
            {
                emissionColor1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, baseColor, float(0)).rgb * float4(0.07450981,0.01568627,0.06623452,1).rgb;
            }
        #else
            emissionColor1 = lerp(1, baseColor, float(0)).rgb * float4(0.07450981,0.01568627,0.06623452,1).rgb;;
        #endif
