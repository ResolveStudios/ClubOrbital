...
void ApplyHoloAlpha(inout float4 color)
{
    ...
    if (float(0) == 0)
    {
        uv = dot(normalize(float4(0,-1,0,1)), poiMesh.worldPos * float(30)) + _Time.x * float(10);
    }
    
    if (float(0) == 1)
    {
        uv = dot(normalize(float4(0,-1,0,1)), poiMesh.localPos * float(30)) + _Time.x * float(10);
    }
    
    if (float(0) == 2)
    {
        uv = dot(float4(0,-1,0,1), poiMesh.uv[0] * float(30)) + _Time.x * float(10);
    }
    float holoRim = saturate(1 - smoothstep(min(float(0.5), float(0.274)), float(0.274), poiCam.viewDotNormal));
