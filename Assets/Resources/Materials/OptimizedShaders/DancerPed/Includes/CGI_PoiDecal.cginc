...
void applyDecals(inout float4 albedo, inout float3 decalEmission)
{
    ...
    decalScale = float4(1,1,0,0);
        ...
        decalColor = POI2D_SAMPLER_PAN(_DecalTexture, _MainTex, uv, float4(0,0,0,0)) * float4(0.01850021,0.02217388,0.02217388,1);
    #else
        uv = decalUV(float(0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float(0));
        decalColor = float4(0.01850021,0.02217388,0.02217388,1);
    #endif
    ...
    decalEmission += decalColor.rgb * decalColor.a * max(float(0) + audioLinkDecalEmission0, 0);
        ...
        decalRotation = 0;
            ...
            #endif
            uv = decalUV(float(0), float4(0.31,0.49,0,0), float(286) + decalRotation, float(0), decalScale, float(0));
        ...
        #endif
        albedo.rgb = lerp(albedo.rgb, customBlend(albedo.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(_DecalBlendAlpha3 + audioLinkDecalAlpha3));
