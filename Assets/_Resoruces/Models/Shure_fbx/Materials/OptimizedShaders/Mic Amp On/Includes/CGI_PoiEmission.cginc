...
float3 calculateEmissionNew(in float3 baseColor, inout float4 finalColor)
{
    ...
    float glowInTheDarkMultiplier0 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0));
        ...
        if (!float(0))
        {
            emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)).rgb * lerp(1, baseColor, float(0)).rgb * float4(0,1,0.3070867,0.392).rgb;
        }
        else
        {
            emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, baseColor, float(0)).rgb * float4(0,1,0.3070867,0.392).rgb;
        }
    #else
        emissionColor0 = lerp(1, baseColor, float(0)).rgb * float4(0,1,0.3070867,0.392).rgb;
    #endif
    ...
    float3 emissionColor1 = 0;
        ...
        emissionStrength1 = float(0);
            ...
            if (poiMods.audioLinkTextureExists)
            {
                ...
                emissionStrength1 += lerp(float4(0,3,0,0).x, float4(0,3,0,0).y, poiMods.audioLink[float(0)]);
